...
RSpec.describe CityHotspot, type: :model do
  ...
  describe "#can_interact?" do
    ...
    it "returns false for a null character" do
      expect(hotspot.can_interact?(nil)).to be false
    end
  end
  ...
  describe "#interaction_blocked_reason" do
    ...
    it "returns a character message for a null character" do
      expect(hotspot.interaction_blocked_reason(nil)).to include("Character")
    end
  end
  ...
  describe "#navigate_url" do
    ...
    it "returns the allowlisted read-only city building route" do
      hotspot.update!(action_type: "open_feature", action_params: {"feature" => "market"})

      expect(hotspot.navigate_url).to eq("/city/buildings/market")
    end

    it "does not infer a feature from the hotspot key" do
      hotspot.update!(key: "market", action_type: "open_feature", action_params: {})

      expect(hotspot.navigate_url).to be_nil
    end
  end

  describe "#world_action_type" do
    it "distinguishes district movement, city exit, and building entry" do
      district = build(:city_hotspot, :district, zone: city_zone)
      gate = build(:city_hotspot, :city_gate, zone: city_zone, destination_zone: destination_zone)
      building = build(:city_hotspot, :read_only_city_building, zone: city_zone)

      expect(district.world_action_type).to eq("city_transition")
      expect(gate.world_action_type).to eq("exit_city")
      expect(building.world_action_type).to eq("enter_city_building")
    end
  end

  describe "constants" do
    it "defines HOTSPOT_TYPES" do
      expect(described_class::HOTSPOT_TYPES).to contain_exactly("building", "district", "exit")
    end
    ...
    it "defines FEATURE_ROUTES" do
      ...
      expect(described_class::FEATURE_ROUTES["market"]).to eq("/city/buildings/market")
