...
| `COMBAT-FIGHT-UI-001` | Active fight composer and state variants | `VERIFICATION_NEEDED` (`Not Done` for 1:1 parity) | Neverlands 1:1 UI/UX parity matrix and Pillar 3 Combat Completion Matrix |
| `COMBAT-LOG-001` | Separate public fight log parity | `VERIFICATION_NEEDED` (`Not Done` for 1:1 parity) | Neverlands 1:1 UI/UX parity matrix and Pillar 3 Combat Completion Matrix |
...
| Binary perks | Required bounded subset | Source perks `7` More Strength and `15` Careful Fighter use the shared save/exclusion flow; their exact Strength and half-wear effects are implemented. | Prerequisites/reset and other named perks remain `[EVIDENCE]`. |
...
- Bounded physical MVP: `VERIFICATION_NEEDED`.
...
|---|---|---|---|---|---|---|
| `COMBAT-ARENA-001` | MVP | Application/start, active, waiting, timeout, surrender, result, explicit finish, and allowlisted return lifecycle | Sufficient for the bounded lifecycle | Implemented and covered through the shared match runtime | `DONE` | — |
| `COMBAT-PVE-PHYSICAL` | MVP | Physical PvE `1x1` and `1xN` through Arena and wilderness | Bounded single- and multi-NPC flows captured | Shared resolver, turn processor, NPC AI, targeting, result, and reload paths are implemented and covered | `VERIFICATION_NEEDED` | Record complete local browser comparisons for one `1x1` and one `1xN` fight, including target switch, finish, return, and reload. |
...
| `COMBAT-TEAM-TURNS` | MVP | Player/team and mixed `1xN` turn synchronization | Side/waiting behavior is sufficient for the bounded flow; reward distribution is tracked separately | All live participants must submit before the shared processor resolves; focused coverage exists | `VERIFICATION_NEEDED` | Record one multi-participant browser flow and confirm target legality, wait release, one shared resolution, result, and reconnect behavior. |
| `COMBAT-TURN-PACKAGE` | MVP | Four legal physical package shapes, one active block, reset/no-op, target selection, AP validation, and multi-attack penalties | Exact source script and live warning behavior captured | Server validation and source-shaped packages are implemented and covered | `DONE` | — |
...
| `COMBAT-FATIGUE` | Full | High-fatigue attack, defense, recovery, and other combat penalties | Exact combat coefficients are missing | World fatigue/action gating exists; no source-derived combat penalty is applied | `EVIDENCE_NEEDED` | Compare otherwise equivalent fights across controlled fatigue bands and capture recovery behavior. |
...
| `COMBAT-XP-GROUP` | Full | General multi-NPC formula and player-group/team XP distribution | General distribution rule is missing | No generalized group distribution is claimed | `EVIDENCE_NEEDED` | Complete the same eligible encounter solo and in a player group, recording participant damage, level, result, and awarded XP. |
...
| `COMBAT-OBSERVATION-DROPS` | Full | Observation modifier, multi-drop behavior, and NPC-specific item/NV probabilities | Exact probability curves are missing | Explicit known probabilities work; unknown entries remain disabled or absent | `EVIDENCE_NEEDED` | Run repeated controlled kills at materially different Observation values and capture all item/NV checks, including zero- and multi-drop results. |
...
| `COMBAT-MAGIC-STATUSES` | Full | Magic damage, resistance, blocks, and persisted status application/expiry | Complete formulas and lifecycle evidence are missing | Bounded allowlisted attack/block rows exist; no generic persisted-status engine is claimed | `EVIDENCE_NEEDED` | Capture one complete flow per magic/status family from selection through resistance, application, repeated-turn effect, expiry, and result. |
...
| `COMBAT-INJURIES` | Full | Ordinary injury chance, type, severity, duration, and Arena risk-field mapping | Taxonomy and selected guaranteed cases are known; ordinary mapping is missing | No ordinary source-derived outcome is inferred | `EVIDENCE_NEEDED` | Capture repeated eligible defeats across Arena risk values and wilderness, including injury display, duration, stacking, and recovery. |
...
| `COMBAT-FIGHT-UI-001` | MVP | Active composer plus waiting, timeout, surrender, victory/defeat, multi-target, and finish states | Required structures and representative states are captured | Responsive surface and functional states exist; complete local/live parity record is missing | `VERIFICATION_NEEDED` | Compare every listed state at desktop width, then verify the same information remains accessible at tablet and mobile widths. |
| `COMBAT-LOG-001` | MVP | Public chronological log, participant summary, pagination, statistics, and shell separation | Public log/stat captures are sufficient | Public responsive routes render the shared structured event stream; fresh geometry/state comparison is missing | `VERIFICATION_NEEDED` | Record a fresh local/source comparison for populated, paginated, statistics, and empty/error states. |
...
| Arena | Documented across arena, combat, live arena captures, and public log captures. | `COMBAT-ARENA-001` and physical `1x1` PvP are `DONE`; PvE/team browser comparison and broader fight-state parity remain `VERIFICATION_NEEDED`. | Follow the exact gates in the Combat Completion Matrix. |
| Combat | Documented across combat reference captures, arena observations, wiki development constants, logs, equipment effects, and the 2026-08-26 live level-17 shield fight. | Bounded physical MVP: `VERIFICATION_NEEDED`. Full Neverlands Combat: `EVIDENCE_NEEDED`. | Use the Combat Completion Matrix; its mechanic rows and exact next gates are canonical. |
...
| Arena | Yes: arena docs, live combat captures, public log captures. | `COMBAT-ARENA-001` and physical `1x1` PvP are `DONE`; multi-participant, PvE, and visual-state checks are `VERIFICATION_NEEDED`. | Follow the Combat Completion Matrix rather than inferring completion from this summary row. |
...
| Combat | Yes: combat captures, public logs, wiki AP/critical/wear/XP constants, item/NV search outputs, magic, equipment effects, and result flow. | Bounded physical MVP: `VERIFICATION_NEEDED`. Full Neverlands Combat: `EVIDENCE_NEEDED`. | Use the canonical Combat Completion Matrix for each mechanic and exact next gate. |
| Arena combat | Yes: arena rooms/applications and NPC training captures. | Bounded lifecycle and physical `1x1` PvP: `DONE`; PvE, team, active-UI, and public-log comparisons: `VERIFICATION_NEEDED`. | Complete the matrix's named browser/source comparisons; formula evidence remains tracked in separate `EVIDENCE_NEEDED` rows. |
