...
village Trading Post share the starter buy/sell/licenses/novice-goods owner.
...
| Professions | Successful gathering explicitly deferred for this task | Separate design owner; World ships empty Look and its 28-second lock, with no profession mutation. | `[EVIDENCE]` capture the alchemy eligibility/tool/yield/counter/failure/interruption loop before implementing successful gathering. |
...
- The outdoor client phase is implemented: `100 x 100` terrain cells, a clipped
  odd-column/row viewport fitted to its equivalent header-plus-main gameplay frame (up to 13 × 7) over a
  15 × 9 buffer, thin red server-offer borders,
...
- Movement completion reloads the bounded map and current-cell presence from
  authoritative state. Timer sleep/Back recovery, rejected-move navigation,
  keyboard focus, and the captured `28`-second Look lock are covered.
...
| --- | --- | --- | --- |
| Login and resume | Yes: live player/location behavior and dashboard-removal decision. | Outdoor cell, exact city node, Frontier Village, village-linked Shop, city Shop, captured read-only interiors, and accessible selected Arena room resume are implemented with sanitized server-side state. Village/Shop resume rechecks the exact active DB-backed entrance cell; Arena resume validates the room and its optional city-region binding. Position changes atomically clear stale room context. | Extend the allowlisted resolver only when another source-backed interior is implemented; never persist arbitrary return URLs. |
...
| Social chat and presence | Yes: live cell/village/Arena-room lists, the Neverlands Chat article, explicit user confirmation of one-cell/room ordinary chat, and mixed personal/world event captures. | Current-cell/room chat uses authenticated bounded polling and locked sends; delivered browser rows survive movement within one login. Personal fight/item/NV events and world announcements retain durable history and Turbo delivery in the same surface. | Exact source online expiry, private/moderation controls, broader announcement operations/event families, and NPC-specific NV probabilities remain evidence gaps. |
...
| Persistence after reload/login | Persist exact region/cell and city-node identity; resume village, city/village Shop, captured read-only buildings, and selected Arena rooms only while still accessible. Coordinate/region changes clear old room state atomically; stale/malformed resume falls back to the unchanged position. World combat now stores an allowlisted World/Character/Inventory finish context while the coordinate remains unchanged; later interiors require their own completed allowlist entry. |
