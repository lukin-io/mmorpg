...
| `COMBAT-PVE-PHYSICAL` | MVP | Physical PvE `1x1` and `1xN` through Arena and wilderness | Bounded single- and multi-NPC flows plus passive current-coordinate entry/return are captured | Shared resolver, turn processor, NPC AI, targeting, result, and reload paths are covered; seeded Chrome completed immediate Arena `1x1` and City-exit/walk/wait wilderness `1x2` through AP reset, target switch, per-NPC search, capped result, one XP/win, Finish, same-cell reload, and duplicate-safety checks | `DONE` | — |
| `COMBAT-WILDERNESS-SELECTION` | Full | Exact passive delay/probability and per-cell eligible opponent/group selection | Current-coordinate availability and variable same-return-context output are confirmed: one `m_1008_1007` chain produced mixed `1x3`, then `1x1`, `1x1`, and mixed `1x2` groups; two no-click intervals were bounded to approximately `230..278` and `127..187` seconds; the complete pool, weights, probability, cooldown, delay distribution, and storage remain unexposed | The bounded local path uses one explicit persisted cell anchor/composition and a provisional server-persisted `10..30`-second due time; it does not yet select variable source-backed groups | `EVIDENCE_NEEDED` | Repeat controlled waits on this and adjacent known hostile/non-hostile coordinates, tabulating every elapsed interval, roster, level, and fight-risk field; inspect only client-exposed payloads, then implement the smallest source-backed pool/weight rule without assuming equal weights. |
...
| `COMBAT-OBSERVATION-DROPS` | Full | Observation modifier, search eligibility, multi-drop behavior, and NPC-specific item/NV probabilities | Exact curves are missing; seven current defeats at Observation `100` emitted no search row at all and therefore are not failed-drop samples | Explicit known probabilities work; unknown entries remain disabled or absent | `EVIDENCE_NEEDED` | Use an NPC that visibly runs a search, then repeat controlled kills at materially different Observation values and capture eligibility, nothing-found, item/NV, and multi-drop results separately. |
...
- a passive check resolves only the persisted source-backed same-cell hostile;
  ...
  fingerprinted due time, and the provisional local delay is not a claimed
  Neverlands probability/timer;
...
| Wild cells | Documented across outdoor movement, exact-current-coordinate hostile observations, composable cell contents, `look`, and fatigue/XP rules. | Fully implemented for the declared World boundary: composed cells, fatigue gate, movement/building/shell interruption, server-persisted passive due state, paired-NPC combat, participant loot, capped solo XP, surrender, duplicate-start protection, and allowlisted return. | Capture one complete gathering/profession outcome plus exact Neverlands passive timing/probability and per-cell roster selection before generalizing authored encounters. |
