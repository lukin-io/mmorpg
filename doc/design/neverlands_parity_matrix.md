...
| Arena | Room/application hub for duels, group fights, NPC training rows, social waiting context, and shared tactical combat. | Route arena entry/return through city buildings and keep application rows compact, side-based, and action-key validated. |
...
| NPCs and quests | NPCs can belong to tiles, city nodes, or buildings; tutorial and story objectives are tied to real action offers. | Author a starter chain teaching move, enter city, enter shop, inventory, combat, skill allocation, and gather. |
| Combat | One AP/body-part/block/log/result contract for arena PvP, arena NPC, wild NPC, and later dungeon fights. | Build the shared turn UI, combat profile, resolver, combat log, NPC response, PvP waiting, timeout, loot check, and finish-result step. |
| Arena combat | Room/application lifecycle feeds the shared combat contract and returns to arena/city context after finish. | Bind NPC training and PvP applications to the same combat profile and result flow. |
...
| Progression, stats, and skills | Profile allocation loop with base vs pending values, numeric skills, boolean perks, explicit save actions, prerequisites, and limited respec. | Make the player profile the primary allocation surface and expose movement/combat effects. |
| Items, inventory, equipment | Item rows show properties, requirements, mass, durability, compact actions, and equipment slot effects. | Connect equipment to combat/vitals/movement, enforce capacity, persist durability, and share item-row behavior with shops. |
