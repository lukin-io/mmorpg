...
| Arena | Partial | Arena rooms, applications, matches, NPC applications, arena services/jobs/channels/specs. | Entry through city arena building/hotspot; room social context; application rows with fight kind/timeout/rules; AP turn combat after accept; combat log as first-class result. | Route arena entry/return through city building flow and tighten arena UI to a dense room/application page. |
...
| Combat | Partial | Turn-based combat models/services/formulas; AP/action validation; body-part targeting concepts; PvE/PvP/arena services; combat logs and specs. | Tight Neverlands-style combat screen with AP choices, target body parts, block assignment, readable round log, resumable state, and return path to world/arena/city. | Standardize one combat UI path and ensure hostile tile/arena entries both create the same auditable combat state. |
| Arena combat | Partial | Arena room/application/match lifecycle, NPC training service, match jobs, channels, UI specs. | City-building entry; waiting room player context; application timeout/fight rules surfaced in the room; return from battle to arena/city context. | Bind arena to city node and align its fight setup with core combat rules. |
| Character vitals | Partial | Character HP/MP fields/services, vitals bars, regen job/channel, battle participants, recovery/infirmary services. | Neverlands-like top-frame HP/MP values and regen timing always visible; regen values passed to UI; combat/recovery affects regen clearly; death/defeat returns through recovery flow. | Make vitals a persistent game-shell component and document exact regen formulas. |
| Progression, stats, and skills | Partial | Character stats/skills, stat allocation, skill allocation, passive skill calculator, Wanderer affects movement travel time, progression services/specs. | Clear categories for combat/peace skills; visible prerequisites and missing requirements; effective vs base skill; trainers and skill unlocks via city/NPC flow; limited expensive respec. | Tie skill training/unlocks to city trainers and expose movement/combat effects in UI. |
| Items, inventory, equipment | Partial | Item templates, inventory, stack/capacity, equipment service, inventory UI/specs, loot generator. | Item requirements/properties visible in shop/inventory; durability; equipment effects on combat/vitals/movement; carried weight feeding travel time; quest-item protection. | Connect weight/encumbrance to `TravelTime` and make item requirements visible in shop/building UI. |
