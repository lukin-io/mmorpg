...
| Items, inventory, equipment | Partial | Item templates, inventory, stack/capacity, equipment service, inventory UI/specs, loot generator; live player capture documents the profile equipment-slot summary and inventory item rows with properties, requirements, mass, durability, and compact actions. | Durable item requirements/properties in item data, durability changes through use/combat, equipment effects on combat/vitals/movement, carried weight feeding travel time, quest-item protection, transfer/gift/sale through economy flow. | Connect weight/encumbrance to `TravelTime`, persist durability/requirements for generated items, and make shop/building item rows share the inventory row contract. |
...
| Dungeons | Required after MVP | Legacy dungeon tables/routes/views and `DungeonInstance` exist, but they model a generic encounter/checkpoint run and are not the design authority. | Source-backed solo/party applications, leader key/unlock, level/equipment eligibility, generated room floors, lamp oil movement, blocking room NPCs, floor-local hidden key/room, chest-or-boss branch, five-seal portal descent, current wiki party-at-portal rule, dungeon inventory, claustrophobia, magical disturbances, deepest and weekly ratings, dungeon currency, specialist shop. | After launch MVP, rebuild dungeon entry and run state from the Neverlands wiki/forum mechanics in `doc/design/features/dungeons.md`; anything not mapped to those sources remains legacy. |
